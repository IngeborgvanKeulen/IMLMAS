...
class LogConverter:
    ...
    def __init__(self, conf_file, sim_name=None):
        ...
        self.main_acct_file = output_conf["main_accounts"]  # Main account list file
    ...
    def get_main_account(self, alert_type, accs):
        # fan in: bene_acct (should be one) is main
        # fan out: orig_acct (should be one) is main
        # gather_scatter: both sender and receiver (should be one)
        # scatter_gather: sender that never receiver (should be one)
        # cycle: max sender is main
        if alert_type == "fan_in":
            return list(set(accs[1]))
        elif alert_type == "fan_out":
            return list(set(accs[0]))
        elif alert_type == "gather_scatter":
            return list(set(accs[1]).intersection(set(accs[0])))
        elif alert_type == "scatter_gather":
            return list(set(accs[0]) - set(accs[1]))
        elif alert_type == "cycle":
            return [accs[0][np.argmax(accs[2])]]
        elif alert_type == "random":
            return [accs[0][np.argmax(accs[2])]]
        elif alert_type == "bipartite":
            return list(set(accs[0]) - set(accs[1]))
        elif alert_type == "stack":
            return list(set(accs[0]) - set(accs[1]))

    def output_main_cases(self):
        """Extract main SAR account list involved in alert transactions from transaction log file
        """
        input_file = os.path.join(self.work_dir, self.alert_tx_file)
        output_file = os.path.join(self.work_dir, self.main_acct_file)
        ...
        for row in reader:
            if len(row) < columns:
                continue
            try:
                alert_id = int(row[indices["alert_id"]])
                type = str(row[indices["alert_type"]])
                sender = int(row[indices["orig_acct"]])
                receiver = int(row[indices["bene_acct"]])
                amount = float(row[indices["base_amt"]])
            except ValueError:
                continue

            if (alert_id, type) in main_accounts:
                main_accounts[(alert_id, type)][0].append(sender)
                main_accounts[(alert_id, type)][1].append(receiver)
                main_accounts[(alert_id, type)][2].append(amount)
            else:
                main_accounts[(alert_id, type)] = ([sender], [receiver], [amount])

        with open(output_file, "w") as wf:
            writer = csv.writer(wf)
            writer.writerow(
                ["ALERT_ID", "MAIN_ACCOUNT_ID", "ALERT_TYPE"])
            for key, accs in main_accounts.items():
                alert_id, alert_type = key
                main_accs = self.get_main_account(alert_type, accs)
                for main_acc in main_accs:
                    writer.writerow((alert_id, main_acc, alert_type))
